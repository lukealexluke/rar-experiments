...
ARXIV_ID_CUTOFF = "2601.16915"
ARXIV_ID_CUTOFF_KEY = (2601, 16915)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Scan the whole TeX source and log line-level arXiv citations "
            "that mention theorem-like locators."
        )
    )
    parser.add_argument(
        "source",
...
def main() -> int:
    ...
    records, stats = collect_line_citation_records(
        tex_text=tex_text,
    ...
    print(
        "Processed "
        f"{stats['citations_seen']} citations, kept {len(records)} lines, "
        f"skipped {stats['skipped_repeat_cite']} repeated citations, "
        f"skipped {stats['skipped_post_cutoff_arxiv_cite']} post-cutoff arXiv citations, "
        f"skipped {stats['skipped_non_arxiv_cite']} for non-arXiv citations, "
        f"skipped {stats['skipped_no_locator']} with no statement locator."
    )
...
def default_output_path(tex_path: Path, output_format: str) -> Path:
    ...
    return logs_root / f"{sanitize_filename(paper_id)}_wholebody{suffix}"
...
def modern_arxiv_sort_key(value: str) -> tuple[int, int] | None:
    normalized = ARXIV_VERSION_SUFFIX_RE.sub("", normalize_arxiv_id(value))
    if not BARE_MODERN_ARXIV_ID_RE.fullmatch(normalized):
        return None
    prefix, suffix = normalized.split(".", 1)
    return int(prefix), int(suffix)


def arxiv_id_is_at_or_before_cutoff(arxiv_id: str) -> bool:
    modern_key = modern_arxiv_sort_key(arxiv_id)
    if modern_key is None:
        return True
    return modern_key <= ARXIV_ID_CUTOFF_KEY


def bib_entry_uses_allowed_arxiv_ids(bib_entry: BibEntry) -> bool:
    return bool(bib_entry.arxiv_ids) and all(
        arxiv_id_is_at_or_before_cutoff(arxiv_id) for arxiv_id in bib_entry.arxiv_ids
    )
...
def collect_line_citation_records(
    tex_text: str,
    bibliography: dict[str, BibEntry],
    require_internal_ref: bool,
) -> tuple[list[LineCitationRecord], dict[str, int]]:
    ...
    line_map: dict[int, LineCitationRecord] = {}
    ...
    for citation in occurrences:
        ...
        line_text = tex_lines[line_number - 1] if line_number <= len(tex_lines) else ""
        if line_number not in line_map:
            line_map[line_number] = LineCitationRecord(line=line_number, text=line_text.rstrip())

    records: list[LineCitationRecord] = []
    for line_number in sorted(line_map):
        line_text = line_map[line_number].text
        # Exclude lines that contain any cite without an explicit locator.
        line_occurrences = list(find_macro_occurrences(line_text, cite_commands))
        if not line_occurrences:
            continue
        if any(not citation_mentions_statement_locator(line_text, occ) for occ in line_occurrences):
            continue
        records.append(line_map[line_number])
        ...
        for citation in cite_occurrences:
            ...
            for key in citation.keys:
                ...
                if bib_entry is None or not bib_entry.arxiv_ids:
                    ...
                    break
                if not bib_entry_uses_allowed_arxiv_ids(bib_entry):
                    post_cutoff_statement = True
                    break
                cited_entries.append({"key": key, "arxiv_ids": bib_entry.arxiv_ids})
            if not keep_statement or post_cutoff_statement:
                break
...
def write_text_log(
    ...
    records: Sequence[LineCitationRecord],
    stats: dict[str, int],
) -> None:
    with output_path.open("w", encoding="utf-8") as handle:
